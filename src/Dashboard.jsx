...
function ExpenseForm({ userId, transactions, onSaved }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      const transaction = { merchant: form.merchant.trim(), category: form.category, date: new Date(`${form.date}T12:00:00`).toLocaleDateString('pt-BR'), initials: form.merchant.trim().slice(0, 2).toUpperCase(), type: categoryType(form.category), kind: form.type, value: valueStr, amount, ...(activeInvoice && activeInvoice.path ? { invoicePath: activeInvoice.path, invoiceName: activeInvoice.name, invoiceProvider: activeInvoice.provider || 'gemini' } : {}) }
  ...
  const buttonLabel = saving ? (invoice && !pendingInvoice ? 'Lendo nota…' : 'Salvando…') : invoice && !pendingInvoice ? 'Ler nota fiscal' : pendingInvoice ? 'Salvar gasto' : 'Adicionar'
  return <form className={styles.expenseForm} onSubmit={submit} ref={formRef}><div className={styles.formGrid}><label className={styles.typeField}>Tipo<select value={form.type} onChange={event => updateField('type', event.target.value)}><option value="gasto">Gasto</option><option value="receita">Receita</option></select></label><label>{form.type === 'receita' ? 'Origem' : 'Estabelecimento'}<input value={form.merchant} onChange={event => updateField('merchant', event.target.value)} placeholder={form.type === 'receita' ? 'Ex.: Empresa X' : 'Ex.: Mercado'} /></label><label>Valor<input value={form.value} onChange={event => updateField('value', event.target.value)} inputMode="decimal" placeholder="0,00" /></label><label>Data<input type="date" value={form.date} onChange={event => updateField('date', event.target.value)} required /></label><label>Categoria<select value={form.category} onChange={event => updateField('category', event.target.value)}>{(form.type === 'receita' ? RECEITA_CATEGORIES : CATEGORIES).map(category => <option key={category}>{category}</option>)}</select></label></div><div className={styles.formActions}><label className={styles.invoiceInput}>Nota fiscal (opcional)<input type="file" accept="image/jpeg,image/png,application/pdf" onChange={event => { void selectInvoice(event.target.files?.[0] || null) }} /></label><button type="submit" disabled={saving}>{buttonLabel}</button></div>{invoice && <small className={styles.fileHint}>Arquivo: {invoice.name}</small>}{duplicate && <div className={styles.analysisWarning} role="alert">Possível lançamento duplicado: <b>{duplicate.merchant}</b> · {duplicate.value} · {duplicate.date}.<br /><span className={styles.dupActions}><button type="button" onClick={confirmDuplicate}>Adicionar mesmo assim</button><button type="button" onClick={resetForm}>Cancelar</button></span></div>}{analysisNotice && <div className={analysisStatus === 'success' ? styles.analysisSuccess : styles.analysisWarning} role="status" aria-live="polite">{analysisNotice}</div>}{error && <div className={styles.authError} role="alert">{error}</div>}</form>
}
...
export function TransactionList({ userId, transactions, onChanged }) {
  ...
  function startEdit(transaction) {
    ...
    setError('')
    setDraft({ merchant: transaction.merchant || '', category: transaction.category || 'Outros', date: toISO(transaction.date), value: Math.abs(parseAmount(transaction)).toFixed(2).replace('.', ','), type: isExpense(transaction) ? 'gasto' : 'receita' })
  }
  ...
  async function saveEdit(event) {
    ...
    setBusy(true)
    try {
      const next = { merchant: draft.merchant.trim(), category: draft.category, date: new Date(`${draft.date}T12:00:00`).toLocaleDateString('pt-BR'), initials: draft.merchant.trim().slice(0, 2).toUpperCase(), type: categoryType(draft.category), kind: draft.type, value: draft.type === 'receita' ? `R$ ${amount.toFixed(2).replace('.', ',')}` : `− R$ ${amount.toFixed(2).replace('.', ',')}`, amount }
...
export default function Dashboard({ userId }) {
  ...
  return <div className={styles.app}><Sidebar activeItem={activeItem} onNavigate={setActiveItem} subscriptionCount={data.subscriptions.length}/><main><Topbar user={data.user} searchQuery={searchQuery} onSearchChange={setSearchQuery} searchOpen={searchOpen} onToggleSearch={() => setSearchOpen(open => !open)} onToggleMenu={() => setMobileMenuOpen(open => !open)}/><div className={styles.content}>{pageContent}</div><footer className={styles.footer}><span><b>FinAI</b> · Automação Financeira</span><span>© {new Date().getFullYear()} FinAI — o radar para as suas finanças</span></footer></main>{mobileMenuOpen && <div className={styles.mobileNavOverlay} onClick={() => setMobileMenuOpen(false)}><div className={styles.mobileNavPanel} onClick={event => event.stopPropagation()}><div className={styles.logo}><span className={styles.logoMark}>✦</span><span className={styles.logoText}>FinAI<small className={styles.logoSub}>Automação Financeira</small></span></div><nav className={styles.nav}>{NAV_ITEMS.map(([label, icon]) => <button key={label} className={activeItem === label ? styles.active : ''} onClick={() => { setActiveItem(label); setMobileMenuOpen(false) }}><span className={styles.navIcon}>{icon}</span>{label}{label === 'Assinaturas' && <span className={styles.navBadge}>{data.subscriptions.length}</span>}</button>)}</nav><button className={styles.sidebarSignOut} onClick={() => signOut(auth)}>Sair da conta</button></div></div>}</div>
}
