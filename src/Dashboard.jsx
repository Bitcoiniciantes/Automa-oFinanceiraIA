...
export default function Dashboard({ userId }) {
  ...
  const pageContent = activeItem === 'Visão geral' ? <><section><div className={styles.dataStatus} aria-live="polite">{dataSource === 'firestore' ? 'Dados sincronizados' : dataSource === 'fallback' ? 'Exibindo dados de demonstração' : 'Carregando dados…'}</div><div className={styles.stats}>{stats.map(stat => <StatCard key={stat.label} {...stat}/>)}</div><ChartPanel transactions={data.transactions}/><div className={styles.lower}><TransactionList userId={userId} transactions={filteredTransactions} onChanged={change => applyChange('transactions', change)}/><CategoryPanel transactions={data.transactions}/></div></section><aside className={styles.side}><SubscriptionRadar userId={userId} subscriptions={data.subscriptions} onChanged={change => applyChange('subscriptions', change)}/><InsightCard insight={insight}/></aside></> : activeItem === 'Meus gastos' ? <section><div className={styles.pageIntro}><h2>Meus gastos</h2><p>Cadastre, edite ou exclua suas transações.</p></div><ExpenseForm userId={userId} transactions={data.transactions} onSaved={transaction => applyChange('transactions', { added: transaction })}/><TransactionList userId={userId} transactions={filteredTransactions} onChanged={change => applyChange('transactions', change)}/></section> : activeItem === 'Assinaturas' ? <section><div className={styles.pageIntro}><h2>Assinaturas</h2><p>Acompanhe, adicione ou edite suas assinaturas.</p></div><SubscriptionRadar userId={userId} subscriptions={data.subscriptions} onChanged={change => applyChange('subscriptions', change)}/></section> : <section><div className={styles.pageIntro}><h2>Assistente IA</h2><p>Faça perguntas sobre seus dados financeiros.</p></div><AssistantPanel data={data} stats={stats} userId={userId}/><div className={`${styles.upgrade} ${styles.upgradeInPage}`}><b>Faça seu dinheiro render</b><p>Receba insights personalizados com o FinAI Pro.</p><button>Conhecer o Pro →</button></div></section>
